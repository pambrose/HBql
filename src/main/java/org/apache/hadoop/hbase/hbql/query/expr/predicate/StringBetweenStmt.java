...
public class StringBetweenStmt extends GenericBetweenStmt<StringValue> {
    ...
    public boolean optimizeForConstants(final Object object) throws HPersistException {
        ...
        if (this.getLower().optimizeForConstants(object))
            this.setLower(new StringLiteral(this.getLower().getCurrentValue(object)));
        else
            retval = false;

        if (this.getUpper().optimizeForConstants(object))
            this.setUpper(new StringLiteral(this.getUpper().getCurrentValue(object)));
        else
            retval = false;
    ...
    public Boolean evaluate(final Object object) throws HPersistException {
        ...
        final String strval = this.getExpr().getCurrentValue(object);
        final boolean retval = strval.compareTo(this.getLower().getCurrentValue(object)) >= 0
                               && strval.compareTo(this.getUpper().getCurrentValue(object)) <= 0;
